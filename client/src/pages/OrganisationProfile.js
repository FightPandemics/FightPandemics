...
const OrganisationProfile = ({ isAuthenticated }) => {
  ...
  const loadNextPage = useCallback(
    ({ stopIndex }) => {
      ...
      console.log({ "next page1 isLoading": isLoading })
      console.log({ "next page1 loadMore": loadMore })
      console.log({ "next page1 stopIndex": stopIndex >= organisationPosts.length })
      console.log({ "next page1 postsLength": organisationPosts.length })
      if (
        !isLoading &&
        loadMore &&
