...
- We are using [i18next](https://www.i18next.com/overview/api) to manage translations on our platform. If you need to add text or change existing text, please do so in the `src/locales/translations/en_US` file. [Lokalise](https://lokalise.com/) is the tool we use to manage our translations. It is configured to automatically pull changes in `en_US` from `staging` branch and edit / add new keys on the Lokalise platform so our volunteers can go and translate the new changes.
