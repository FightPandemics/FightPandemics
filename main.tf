...
module "main" {
  ...
  backend_env_variables = [
    ...
    {
      name  = "MONGO_URI"
      value = "mongodb+srv://${data.aws_ssm_parameter.db_user.value}:${data.aws_ssm_parameter.db_password.value}@${data.aws_ssm_parameter.db_host.value}/tempformultiauth?retryWrites=true&w=majority"
    },
    {
      name  = "APP_DOMAIN"
