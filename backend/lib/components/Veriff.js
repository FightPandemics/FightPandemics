...
const validateWebhookEvent = async (req, reply) => {
  ...
  if (PUBLIC_KEY !== incomingXAuthClient) {
    req.log.err(`staging debug: 403 details 1 - ${PUBLIC_KEY}, ${incomingXAuthClient}`);
    throw false;
  }
  req.log.err(`staging debug: working 1 - ${PUBLIC_KEY}, ${incomingXAuthClient}`);
  ...
  if (xSignature != incomingXSignature) {
    req.log.err(`staging debug: 403 details 2 - ${xSignature}, ${incomingXSignature}`);
    throw false;
  }
  req.log.err(`staging debug: working 2 - ${xSignature}, ${incomingXSignature}`);
};
